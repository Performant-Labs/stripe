...
class SimpleCheckoutForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    if ($this->checkTestStripeApiKey()) {
      ...
      if ($charge) {
        \Drupal::messenger()->addMessage('Charge status: ' . $charge->status);
        if ($charge->status == 'succeeded') {
          $link_generator = \Drupal::service('link_generator');
          \Drupal::messenger()->addMessage($this->t('Please check payments in @link.', [
  ...
  private function createCharge($stripe_token, $amount) {
    try {
      ...
      \Drupal::messenger()->addMessage($this->t('Stripe error: %error', ['%error' => $e->getMessage()]), 'error');
    }
  }
