...
class Stripe extends Hidden {
  ...
  public static function processStripe(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    if (empty($apikey)) {
      ...
      \Drupal::messenger()->addMessage(t('You must <a href="@settings_uri">configure</a> the <b>Publishable API Key</b> on <b>%environment</b> for the stripe elements to work.', ['%environment' => $config->get('environment'), '@settings_uri' => $settings_uri]), 'error');
    }
    $settings = [];
